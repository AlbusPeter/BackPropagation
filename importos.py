...
def Getdata():
    ...
    testing_labels = [d[0] for d in testing_data]
    training_labels_oneHot = np.zeros([len(training_labels), 10])
    testing_labels_oneHot = np.zeros([len(testing_labels), 10])
    training_labels_oneHot[np.arange(len(training_labels)), training_labels] = 1
    testing_labels_oneHot[np.arange(len(testing_labels)), testing_labels] = 1

    training_data = zip(training_pixels, training_labels_oneHot)
    testing_data = zip(testing_pixels, testing_labels_oneHot)
